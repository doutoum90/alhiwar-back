...
export class DbAdminService {
  ...
  async getStats() {
    ...
    // Compteurs (adapte si tes tables ont d'autres noms)
    ...
    // last backup stocké en fichier optionnel (ou table)
  ...
  async backup(): Promise<{ ok: true; backupAt: string; file: string }> {
    ...
    // Optionnel : exposer la date via env (ou une table)
  ...
  async optimize(): Promise<{ ok: true }> {
    // VACUUM (attention : certains VACUUM ne passent pas dans transaction)
    await this.dataSource.query(`VACUUM (ANALYZE);`).catch(async () => {
      // fallback si VACUUM bloqué
  ...
  async cleanupLogs(): Promise<{ ok: true }> {
    // Si tu as une table "logs" / "audit_logs", tu peux adapter ici.
    // Exemple : supprimer logs > 30 jours
